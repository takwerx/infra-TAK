...
if(document.getElementById('tak-cert-password-inline')||document.getElementById('deploy-cert-password-inline')){loadTakCertPassword();}
...
// v10.1.29: /api/takserver/vacuum now preflights VACUUM FULL (400 when there is
// not enough free disk to rewrite the largest table, 409 warn_no_bloat when there
// is nothing to reclaim). The 409 is advisory, so keep an explicit override here —
// otherwise the gate would make Compact Database unreachable from this page.
async function runVacuum(full,force){var msg=document.getElementById('vacuum-msg');var out=document.getElementById('vacuum-output');if(full&&!force&&!confirm("VACUUM FULL locks the CoT tables. Run when TAK Server is not running. Continue?"))return;if(out){out.style.display='none';out.textContent='';}_vacuumSetUI(true,full,0);try{var r=await fetch('/api/takserver/vacuum',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({full:full,force:force===true})});var d=await r.json();if(d.success||d.status==='running'){_pollVacuumStatus();}else{_vacuumSetUI(false,false,0);if(out){out.style.display='block';out.textContent=d.error||'Failed';}if(msg){msg.style.color=d.warn_no_bloat?'var(--yellow)':'var(--red)';msg.textContent=d.warn_no_bloat?'Compacting will not help here. ':(d.error||'Failed');if(d.warn_no_bloat){var b=document.createElement('button');b.className='btn btn-ghost';b.style.cssText='padding:2px 10px;font-size:11px;margin-left:6px';b.textContent='Run anyway';b.onclick=function(){runVacuum(true,true);};msg.appendChild(b);}}}}catch(e){if(msg){msg.textContent='Error: '+e.message;msg.style.color='var(--red)';}_vacuumSetUI(false,false,0);}}
...
function pollDeployLog(){
    ...
    const poll=async()=>{
        try{const r=await fetch('/api/deploy/log?after='+logIndex);const d=await r.json();pollFails=0;
            ...
            else if(d.complete){const b=document.getElementById('deploy-btn');if(b){b.textContent='\u2713 Deployment Complete';b.style.background='var(--green)';b.style.opacity='1'};const dl=document.getElementById('cert-download-area');if(dl)dl.style.display='block';loadTakCertPassword();var wa=document.createElement('div');wa.style.cssText='background:rgba(59,130,246,0.1);border:1px solid var(--border);border-radius:10px;padding:20px;margin-top:20px;text-align:center';var wt=document.createElement('div');wt.style.cssText='font-family:JetBrains Mono,monospace;font-size:14px;color:#06b6d4;margin-bottom:12px';wt.textContent='\u23f3 TAK Server needs ~5 minutes to fully initialize before login will work.';var wb=document.createElement('button');wb.textContent='Refresh Page';wb.style.cssText='padding:10px 24px;background:linear-gradient(135deg,#1e40af,#0e7490);color:#fff;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer';wb.onclick=function(){window.location.href='/takserver'};wa.appendChild(wt);wa.appendChild(wb);document.getElementById('deploy-log-area').after(wa)}
