...
const FN_PARSE_COT = [
  ...
  "var _SIMP_TOL = (cfg.simplifyTolerance != null && cfg.simplifyTolerance !== '') ? Number(cfg.simplifyTolerance) : 0.005;",
  "var _SIMP_MIN = 60;",
  ...
  "  if (!_SIMP_TOL || !pts || pts.length <= _SIMP_MIN) return pts;",
  "  var out = _dp(pts, _SIMP_TOL);",
  "  if (isRing) {",
...
"  var _hash = djb2(hp.join('|'));",
  ...
  "      // Shape-aware reduction: escalate the DP tolerance until under the cap, instead of",
  "      // naive every-Nth resampling (which drops corners and looks blocky). Endpoints are",
  "      // preserved by _dp, so a closed ring stays closed.",
  "      var _t2 = (_SIMP_TOL || 0.001) * 2; var _g = 0; var _lastV = ring2[ring2.length-1];",
  "      while (ring2.length > MAX_VERTS && _g < 40) { ring2 = _dp(ring2, _t2); _t2 *= 1.5; _g++; }",
  "      if (_lastV && (ring2[ring2.length-1][0] !== _lastV[0] || ring2[ring2.length-1][1] !== _lastV[1])) ring2.push(_lastV);",
  "    }",
