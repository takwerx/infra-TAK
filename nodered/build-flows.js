...
const FN_PARSE_COT = [
  ...
  "// Per-polygon POINT BUDGET — cap each ring/path at this many points. A shape already under the",
  "// budget is left untouched (small/curvy fire perimeters keep full detail — never chopped); only",
  "// big shapes (WWA/NWS zones, thousands of verts) get reduced down to it. Caps total bytes so dense",
  "// feeds can't stall CloudTAK, and reduces by POINT COUNT (not absolute distance) so shape size is",
  "// respected automatically. Fleet constant — same on every box, no per-feed tuning.",
  "var _PT_BUDGET = 100;",
  ...
  "  return out;",
  "}",
  "function _simp(pts, isRing) {",
  "  if (!pts || pts.length <= _PT_BUDGET) return pts;",
  "  // Over budget: escalate the DP tolerance until at/under the point budget (shape-aware — keeps",
  "  // corners, drops the least-significant points first). Endpoints are preserved by _dp.",
  "  var out = pts, _t = 0.0002, _g = 0;",
  "  while (out.length > _PT_BUDGET && _g < 40) { out = _dp(out, _t); _t *= 1.5; _g++; }",
  "  if (isRing) {",
...
"  var _hash = djb2(hp.join('|'));",
  ...
  "      _parts.push({ uid: _isMultiPoly ? uid+'-r'+_ri2 : uid, _hash: _isMultiPoly ? djb2(_hash+'|r'+_ri2+'|'+_rgk) : _hash, lat: _rsy/_rr2.length, lon: _rsx/_rr2.length, verts: _simp(_rr2, true) });",
  "    }",
  ...
  "      _parts.push({ uid: _isMultiPath ? uid+'-p'+_pi : uid, _hash: _isMultiPath ? djb2(_hash+'|p'+_pi+'|'+_pgk) : _hash, lat: _psy/_pp.length, lon: _psx/_pp.length, verts: _simp(_pp, false) });",
  "    }",
  "  } else {",
  ...
  "  if (!noColor) detail.strokeWeight = [{ _attributes: { value: String(fClsWeight) + '.0' } }];",
