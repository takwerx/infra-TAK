...
function makeEngineTab(feed) {
  ...
  const FN_RECONCILE = [
    ...
    "  var MASS_MISS_THRESHOLD = MISS_THRESHOLD * 2;",
    "  var candidates = [];",
    ...
    "  var needMisses = massDelete ? MASS_MISS_THRESHOLD : MISS_THRESHOLD;",
    "",
    "  for (var ci = 0; ci < candidates.length; ci++) {",
    "    var duid = candidates[ci];",
    "    misses[duid] = (misses[duid] || 0) + 1;",
    "    // W2 hysteresis (+ W4's longer bar when the whole pass looks like a mass delete).",
    "    if (!oneShotPurge && misses[duid] < needMisses) { nHeld++; continue; }",
    "    delete misses[duid];",
    "    nDel++;",
    ...
    "      + ' until the claim repeats across ' + MASS_MISS_THRESHOLD + ' polls');",
    "  }",
    ...
    "    if (arcgis[tu] || existing[tu]) { delete tombs[tu]; continue; }  // never kill a live shape",
    "    sweepUids.push(tu);",
    "  }",
