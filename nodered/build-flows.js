...
const configFlows = [
  ...
  },
  {
    ...
    func: [
      ...
      "  headers: { 'accept': '*/*' }",
      "});",
      "// TAK 5.7 (RELEASE8): createMission NPEs ('bytes is null' -> 500 code 6, empty",
      "// message) when the PUT has NO body — older TAK accepted a body-less create.",
      "// Always send '{}': 5.7 needs it, older versions ignore it. (v10.1.25 W4)",
      "var createBody = '{}';",
      ...
      "  headers: { 'accept': '*/*', 'Content-Type': 'application/json', 'Content-Length': String(createBody.length) }",
      "});",
      ...
      "    return tryRequest(putOpts, createBody).then(function(putRes) {",
...
const FN_PARSE_COT = [
  ...
  "  if (!pts || pts.length <= _PT_CAP) return pts;",
  ...
  "    if (r.length > _PT_CAP) { lo = mid; } else { hi = mid; out = r; }",
  "  }",
  "  if (isRing) {",
...
function makeEngineTab(feed) {
  ...
  const FN_RECONCILE = [
    ...
    "      nDel++;",
    "      tombs[uid] = Date.now();  // v10.1.5 W4: swept as ForceDelete below + every poll for 14d",
    "      node.send([null, {",
    "        method: 'DELETE',",
    ...
    "  var sweepUids = [];",
    "  for (var tu in tombs) {",
    "    if (_now2 - tombs[tu] > TOMB_TTL) { delete tombs[tu]; continue; }",
    "    if (arcgis[tu]) { delete tombs[tu]; continue; }  // entity came back — never kill a live shape",
    "    sweepUids.push(tu);",
    "  }",
