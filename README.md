...
**Latest release: [v0.9.25-alpha](docs/RELEASE-v0.9.25-alpha.md)** — Drop-in hotfix. Heals a self-inflicted YAML breakage that left **Sync webadmin to Authentik** stuck behind a red `yaml: mapping key "cap_drop" already defined` banner. Adds an unconditional PyYAML-based compose-YAML self-heal at every console boot (resolves duplicate mapping keys via last-wins), wires it into Sync webadmin clicks and a new `POST /api/authentik/compose-heal` operator endpoint, and replaces the YAML validator's previous `docker compose config` shell-out (which false-negativ'd on env-var interpolation) with a PyYAML strict reparse that checks structure without runtime dependencies. Six in-release hotfixes ended with a single line from the operator: "ok it worked." See the [release notes](docs/RELEASE-v0.9.25-alpha.md) for the full forensic trace. Older releases on the [GitHub Releases tab](https://github.com/takwerx/infra-TAK/releases).
