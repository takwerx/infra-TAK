...
**Latest release: [v0.9.27-alpha](docs/RELEASE-v0.9.27-alpha.md)** — Architectural fix for v0.9.26's silent fleet-fracture bug. v0.9.26's `_ensure_authentik_pgbouncer_pool_size` used `max(cur, target)` to "preserve operator overrides" — which silently let `tak-10` keep its operator-typed 250/50 while `test8` got the codified 75/15 → test8 hit `query_wait_timeout` storms within 5 min of pulling the release. **The 65-min validation I did on tak-10 before shipping was on a configuration the codebase never produced.** v0.9.27 replaces the constant with a **deterministic autotuner**: pool size is a function of observed `pg_stat_activity` load on each box, computed identically everywhere (`target_ceiling = max(FLOOR=90, peak × 2.0), capped at 60% of PG max_connections; default:reserve = 5:1 split`). The watchdog already samples idle conns every 2 min; v0.9.27 stores those in `settings.pool_autotune.samples` (last 30 ≈ 1 h). On every console boot, `_ensure_authentik_pgbouncer_pool_size` calls the autotuner and **always writes** the result to compose YAML — no `max(cur, target)`. Cold-start safety: first v0.9.27 boot takes a one-shot live `pg_stat_activity` reading so tak-10 (steady-state ~180 idle) lands at its current 250/50 (cap hit) without flapping, while test8 (peak ~85 idle) converges to ~142/28. New `.cursor/rules/fleet-uniform-config.mdc` codifies the principle — no `max(cur, target)`-style override-preservation in any migration, ever; minimum 3-box dev-channel validation before main merge. **Awaiting multi-box validation on tak-10 + test8 + responder before main release.** See the [release notes](docs/RELEASE-v0.9.27-alpha.md) and [plan](docs/PLAN-v0.9.27-alpha.md) for the full forensic + validation gate.
...
### v0.9.27-alpha — 2026-05-17 — PGBOUNCER POOL AUTOTUNE (KILL v0.9.26 FLEET FRACTURE)

**Headline: replaces v0.9.26's `max(cur, target)` "preserve operator override" semantic — which silently let `tak-10` keep its operator-typed 250/50 pool while `test8` got the codified 75/15 and hit `query_wait_timeout` storms within 5 min — with a deterministic autotuner. Pool size is now a function of observed `pg_stat_activity` load on each box, computed identically everywhere. Same logic → same load → same config. No operator-typed numbers persisted. Awaiting multi-box validation on tak-10 + test8 + responder before main merge.**

- **Root cause of v0.9.26 fracture.** `_ensure_authentik_pgbouncer_pool_size` did `new_default = max(cur_default, target)` to "never lower an operator override." That preserved tak-10's manual 250/50 (set during 2026-05-17 morning's troubleshooting) on first v0.9.26 migration → tak-10 stayed at 250/50. test8 had no override → migration applied codified 75/15 literally → 297 `query_wait_timeout` events in 5 min, server-1 `(unhealthy)`. I validated tak-10's 65 min stability before shipping v0.9.26 to main — but tak-10 was running 250/50; the codebase ships 75/15. They're not the same release. **No box anywhere actually ran the codebase default before I shipped it.**
- **Architectural fix (autotune).** Pool size becomes a deterministic function: `peak = max(idle_count over last 30 watchdog samples ≈ 1 h)`; `target_ceiling = max(FLOOR=90, ceil(peak × 2.0))`; capped at `PG_MAX_CONN=500 × 0.6 = 300`; split 5:1 default:reserve. Same logic on every box. Migration **always writes** the autotune output to compose YAML — no `max(cur, target)`. An operator-typed pool value gets reconciled to the autotune decision on next migration tick.
- **Sampling.** New `_authentik_pool_autotune_sample()` appends `{t, idle, channels, dramatiq, cache, advisory_lock, other}` to `settings.pool_autotune.samples` (ring buffer, last 30). Called from the existing watchdog loop every 2 min. `_classify_idle_load()` now runs on every tick (was alert-paths only); same classification feeds both the watchdog log and the autotune sampler.
- **Cold-start safety.** New `_authentik_pool_autotune_seed_from_live_pg()` takes a one-shot `pg_stat_activity` reading when the samples ring is empty (the FIRST v0.9.27 migration on any v0.9.26 box). Without it, tak-10 would shrink 250/50 → FLOOR 75/15 on first boot and immediately re-trigger the storm we're fixing. With it: seed picks up tak-10's steady-state ~180 idle → target ceiling = 300 (cap hit), default=250, reserve=50, **matches the current compose** → no pgbouncer recreate happens. test8 (peak ~85 idle) lands at ~142/28 — autotune **shrinks** the pool from my manual 250/50 to what test8's actual load justifies.
...
- **Auditability.** Every autotune decision recorded in `settings.json.pool_autotune.last_decision` with `peak_observed`, `target_*`, `reason` (one-line human string), `samples_seen`, `capped_at_pg_max`, `computed_at`, `version`. `last_applied` records the actual compose write (or `noop:true` if it matched). Operators can `diff` decision-vs-runtime at any time.
- **New cursor rule `.cursor/rules/fleet-uniform-config.mdc`.** Codifies the fleet-uniform principle: codebase produces the same operational state on every box (constant or autotune output, never operator-typed). Forbids `max(cur, target)` override-preservation in any config migration. Requires minimum 3-box dev-channel validation + 60 min soak before any main merge that touches fleet-wide configuration.
- **All v0.9.26 hotfix fixes preserved.** Tasklog purge (initial + Hotfix #2 deep compaction), `vm.overcommit_memory=1`, watchdog class breakdown — all unchanged. Only the pool-sizing migration semantic is rewritten.
- **Dev-channel ship.** v0.9.27 stays on `dev` until validated on all 3 test boxes (tak-10, test8, responder) per `docs/PLAN-v0.9.27-alpha.md`. No tag pushed until validation passes.

Full notes: [docs/RELEASE-v0.9.27-alpha.md](docs/RELEASE-v0.9.27-alpha.md). Plan: [docs/PLAN-v0.9.27-alpha.md](docs/PLAN-v0.9.27-alpha.md).
