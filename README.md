...
**Latest release: v0.9.20-alpha** — **MediaMTX HLS COOKIE-CHECK FIX + CADDYFILE AUTO-REGEN ON UPDATE NOW + AUTHENTIK POSTGRESQL POOL TUNING + GUARD DOG 8089 FALSE-POSITIVE FIX.** Drop-in update — Update Now is the only step (Guard Dog scripts auto-update on console startup). Four coordinated fixes shipping together: (1) **MediaMTX HLS** — `header_down Location ^ /hls-proxy` added to the `handle_path /hls-proxy/*` block in `generate_caddyfile()`, so MediaMTX v1.18.x's HLS cookie-check redirect lands back inside Caddy's proxy handler instead of 404'ing. Without this, HLS Watch is broken on every infra-TAK server running MediaMTX v1.18.0+. Reference: [INFRA-TAK-CADDY-HLS-v1.18.md](https://github.com/takwerx/mediamtx-installer/blob/main/docs/INFRA-TAK-CADDY-HLS-v1.18.md). (2) **Caddyfile auto-regenerate** — `_post_update_auto_deploy()` now calls `generate_caddyfile() + systemctl reload caddy` as its last step, so future Caddyfile template changes ship via Update Now alone without operators having to click Reload manually. (3) **Authentik PostgreSQL pool** — `max_connections` bumped 300 → 500 in the Postgres command line, `AUTHENTIK_POSTGRESQL__CONN_MAX_AGE=60` + `AUTHENTIK_POSTGRESQL__CONN_HEALTH_CHECKS=true` appended to `~/authentik/.env` (Django persistent connections, ~95% handshake-churn reduction), and a `django_channels_postgres_message` size signal added to the spiral monitor's evidence dict (informational). Addresses `FATAL: sorry, too many clients already` and idle-in-tx pileup on the django_postgres_cache table observed on tak-10 after the v0.9.18 LDAP-routing fix. The PG-pool helpers are wired into BOTH `_post_update_auto_deploy()` and the unconditional `_startup_migrations` block — so a "burned VERSION string" (same-VERSION dev-channel pull pattern) cannot silently skip the migration. Refs: [goauthentik/authentik#20714](https://github.com/goauthentik/authentik/issues/20714), [#20644](https://github.com/goauthentik/authentik/issues/20644), [Authentik configuration docs](https://docs.goauthentik.io/install-config/configuration/). (4) **Guard Dog 8089 false-positive fix ([#24](https://github.com/takwerx/infra-TAK/issues/24))** — `nc -z 127.0.0.1:8089` TCP-connect liveness probe replaced with an `ss -ltn "sport = :8089"` LISTEN check. Port 8089 uses mutual TLS; raw TCP probes trigger Netty SSL errors and on some netcat-openbsd builds return non-zero, draining the 3/day restart cap and spamming `SKIP | 8089 unhealthy but daily restart cap (3) reached` in the Guard Dog activity log. Same `nc -z` → `ss -ltn` change in `tak-post-start.sh`'s boot-wait loop. Scripts auto-deploy to `/opt/tak-guarddog/` on console startup (`Guard Dog: 2 script(s) updated on console startup.` log line) — no Deploy Guard Dog button click required. All migrations idempotent + smoke-tested against 13 synthetic compose/.env cases (including operator-set `max_connections>500` and partial pre-existing env keys — no downgrades, no overwrites). **Fully validated on tak-10** (`test12.taktical.net`, MediaMTX v1.18.1) 2026-05-13: HLS Watch playback ✓, `max_connections=500` ✓, `CONN_MAX_AGE=60` + `CONN_HEALTH_CHECKS=true` on server+worker ✓, PG connections 157/500 with 0 idle-in-tx (was 221 total / 75 idle-in-tx), `channels_postgres_message` 109,840 rows / 61 MB → 3,834 rows / 40 MB (97% rowcount reduction), Authentik server CPU 807% → 29%, Guard Dog 8089 watch exit=0 with failcount=0, zero new errors in worker log for 18+ min post-deploy. See `docs/RELEASE-v0.9.20-alpha.md` for full details. — *Previously, v0.9.18-alpha:* **AUTHENTIK LDAP ROUTING REPAIR HOTFIX.** Drop-in update — Update Now is the only step. v0.9.18 fixes a critical bug in the v0.8.4/v0.8.5 LDAP outpost routing repair (both reactive `_apply_authentik_ldap_routing_repair` and proactive `_ensure_authentik_ldap_outpost_on_fqdn`) where a single-pass rewriter inserted a second `extra_hosts:` key whenever the LDAP block already had one — making the resulting `docker-compose.yml` unparseable (`mapping key "extra_hosts" already defined`) and trapping spiraling boxes in a forever repair-rollback loop (10-minute cycles). Observed in the wild on tak-10: 492% Authentik CPU, 74 `failed to execute flow` events/min, ~213 PostgreSQL connections from authentik-server — repair attempted 6 times in 70 min, every attempt rolled back. The rewriter is now a shared helper (`_rewrite_ldap_compose_to_fqdn`) that pre-scans the LDAP block once to detect existing `extra_hosts:` and only injects when truly missing. Validated against 5 real-world compose states (tak-10 stuck state, clean install, no LDAP block, corrupt LDAP image, already on FQDN) — produces clean valid YAML in every case and is fully idempotent. See **[docs/RELEASE-v0.9.18-alpha.md](docs/RELEASE-v0.9.18-alpha.md)** for the full notes. Prior releases: [v0.9.17](docs/RELEASE-v0.9.17-alpha.md) (Authentik Redis self-heal + Caddy update fix), [v0.9.16](docs/RELEASE-v0.9.16-alpha.md) (Authentik worker CPU hotfix — stale Local Docker service connection; Caddy update button + Authentik update spinner), [v0.9.15](docs/RELEASE-v0.9.15-alpha.md) (TAK Portal admin-account guardrail — `akadmin`/`webadmin` hidden + action-locked), [v0.9.14](docs/RELEASE-v0.9.14-alpha.md) (Authentik admin recovery hotfix — status-read ak-shell fallback so the panel works when the bootstrap token is 403-locked), [v0.9.13](docs/RELEASE-v0.9.13-alpha.md) (Authentik admin recovery — initial feature; superseded by v0.9.14 if you hit the 403 read-path bug), [v0.9.12](docs/RELEASE-v0.9.12-alpha.md) (cyber security hardening — port-exposure lockdown + post-auth route fixes + ReputationPolicy `negate=True` fix; **important if you haven't updated past v0.9.11 yet**), [v0.9.11](docs/RELEASE-v0.9.11-alpha.md) (security hotfix — CloudTAK PG_MEM/PGMiner; **read this if you haven't updated past v0.9.10 yet**), [v0.9.10](docs/RELEASE-v0.9.10-alpha.md), [v0.9.9](docs/RELEASE-v0.9.9-alpha.md), [v0.9.8](docs/RELEASE-v0.9.8-alpha.md), [v0.9.7](docs/RELEASE-v0.9.7-alpha.md), [v0.9.6](docs/RELEASE-v0.9.6-alpha.md), [v0.9.5](docs/RELEASE-v0.9.5-alpha.md), [v0.9.4](docs/RELEASE-v0.9.4-alpha.md), [v0.9.3](docs/RELEASE-v0.9.3-alpha.md), [v0.9.2](docs/RELEASE-v0.9.2-alpha.md), [v0.9.1](docs/RELEASE-v0.9.1-alpha.md), [v0.9.0](docs/RELEASE-v0.9.0-alpha.md) — older releases on the [GitHub Releases tab](https://github.com/takwerx/infra-TAK/releases).
