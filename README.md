...
**Latest release: [v0.9.26-alpha](docs/RELEASE-v0.9.26-alpha.md)** — Drop-in hotfix. Restores the v0.9.24-era "Authentik is perfectly quiet" baseline that the v0.9.25 release-day restart-churn arc surfaced (but did not cause) was no longer holding on `tak-10`. Closes a long-running silent failure in the **weekly `takauthentiktasklogpurge.service` Guard Dog timer** that has been failing every Sunday for ~6 months with `ERROR: VACUUM cannot run inside a transaction block` — combined with a hardcoded 30-day delete window that didn't match release-day churn bloat, `authentik_tasks_*` tables grew to ~960 MB / 2.66M rows in under 24h and tipped `authentik-server-1` into a sustained ~80% CPU request-cancellation storm. Three coordinated fixes: a canonical multi-tier purge script (7d → 24h → 1h escalation, separate `psql -c` for DELETE vs VACUUM), an `_ensure_authentik_tasklog_purge_script` auto-heal that overwrites the broken on-disk script on every console boot, and an `_auto_authentik_tasklog_purge` inline Python cleanup wired into `_startup_migrations` so we never depend on a single-point-of-failure timer for Authentik hygiene again. v0.9.25's `cap_drop` heal is unchanged. See the [release notes](docs/RELEASE-v0.9.26-alpha.md) for the full forensic trace + tak-10 evidence. Older releases on the [GitHub Releases tab](https://github.com/takwerx/infra-TAK/releases).
