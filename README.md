...
**Latest release: [v0.9.29-alpha](docs/RELEASE-v0.9.29-alpha.md)** — Security audit touch-up (Project Shakespear 2026-04) + Authentik MAX_REQUESTS cascade short-circuit + TAK LDAP CoreConfig verifier namespace fix + cloud public-IP detection + MediaMTX Caddy-LE cert read-access fix + **8446 webadmin-login-fails root-cause fix (`ns0:` namespace prefix in CoreConfig.xml broke Spring Security LDAP wiring)** + **mediamtx-webeditor `PermissionError` fail-loop fix (stream.* returning Authentik "Not Found" was actually port 5080 down, not Authentik 404)**. A friend-of-the-operator audit (Oblivion Edge Vulnerability Research LLC, 74 findings, 9 CRITICAL) was cross-referenced against current `dev` — most of the CRITs target MediaMTX (different repo, forwarded), CRIT-06 cited stale line numbers that are already mitigated, but **CRIT-08 (`StrictHostKeyChecking=no` in 9 guarddog SSH calls) was real and unfixed**. v0.9.29 ships: (1) **CRIT-08 fix** — 9 guarddog scripts switched to `StrictHostKeyChecking=accept-new` with pinned `/opt/tak-guarddog/known_hosts`; 2 raw-SSH callsites in `app.py` also migrated; `known_hosts` file auto-provisioned on Guard Dog deploy + settings sync. (2) **CRIT-07 cleanup** — all 3 `eval "$(python3 ...)"` patterns in guarddog scripts replaced with newline-delimited `read`; `eval "$cmd"` in `remote_cmd()` replaced with `bash -c`. (3) **MED `atakatak` nag** — dismissable orange banner across every console page when the TAK cert export password is still the upstream default; auto-clears the dismiss flag when a non-default password is later saved. (4) **LOW CSP tightening** — `'unsafe-eval'` dropped from `script-src` (codebase has zero `eval` / `new Function` / string-form `setTimeout`). (5) **`docs/SECURITY-AUDIT-RESPONSE-2026-04.md`** — full 388-line per-finding response with CRIT-06 mitigation map, already-addressed-defaults table (CSRF, session flags, persistent secret_key, rate limiting, security headers), and outstanding work for v0.9.30+. (6) **Authentik MAX_REQUESTS cascade short-circuit (post-v0.9.28 stability)** — field forensic on tak-10 2026-05-18 caught the v0.9.23 autotuner climbing back from floor (100) to baseline (1000) at +25%/30min, producing **10 unnecessary `authentik-server-1` + `authentik-worker-1` recreates over ~7 hours** after PgBouncer absorbed the underlying leak pressure (each recreate drops LDAP outpost websocket 1-5s, visible as Caddy 502s). New idempotent startup migration `_patch_authentik_max_requests_snap_to_baseline_if_pgbouncer` snaps MAX_REQUESTS straight to baseline in **one recreate** when PgBouncer is installed AND no SAFETY NET fires in last 6h AND no autotune apply in last 30min — closing the gap left by `_ensure_authentik_pgbouncer`'s post-install reset (which only fires on fresh installs, not on upgrades-while-stuck-at-floor). Silent no-op on boxes already at baseline (all three current dev boxes). (7) **`.cursor/rules/no-main-merge-no-tag-without-permission.mdc`** — new rule codifying the boundary established during v0.9.28-alpha shipping: AI agents must always ask for explicit operator green-light before merging to `main` or pushing tags. (8) **TAK LDAP CoreConfig verifier — namespace-prefix tolerance** — discovered 2026-05-18 on a fresh Azure deploy `tak-test-4` (v0.9.28 main): LDAP connect-sync banner reported `coreconfig=FAIL ready=NO` while `ldapsearch` against `cn=adm_ldapservice` actually succeeded against the running OpenLDAP. Root cause: TAK Server 5.7-RELEASE-32-HEAD canonicalizes `CoreConfig.xml` on restart with the `ns0:` XML namespace prefix (`<ns0:auth ...>...</ns0:auth>`), and `_coreconfig_has_ldap()` was doing naive `content.lower().find('<auth')` substring lookup — false-negative. Fixed: both the verifier and the legacy text-fallback patcher `_apply_coreconfig_ldap_auth_text()` now match `<(?:[A-Za-z][\w-]*:)?auth\b` (and `<ldap>` siblings) with a namespace-tolerant regex. Unit-tested against 5 XML shapes (canonical-ns / plain / default=file / leftover-ldap-element / no-auth) — all 5 pass. LDAP functionality was never broken; only the verification was. (9) **Cloud public-IP detection on fresh install** — same `tak-test-4` deploy surfaced that `start.sh` used `hostname -I | awk '{print $1}'` to populate `settings.server_ip` and the SSL self-signed cert SAN, which on Azure/AWS/GCP returns the *private* interface IP. New `detect_server_ip()` helper queries Azure Instance Metadata Service → AWS IMDSv1 → `api.ipify.org` → `ifconfig.me` → falls back to `hostname -I`, with IPv4 dotted-quad validation between each step. Used at both the settings.json write and the openssl SAN-generation sites. Existing installs aren't auto-rewritten — operators on cloud VMs with a private IP in Settings → Server IP should paste the public IP once and Save. See [release notes](docs/RELEASE-v0.9.29-alpha.md) for the full per-item rationale + the validation gate.

**Previous release: [v0.9.28-alpha](docs/RELEASE-v0.9.28-alpha.md)** — Enterprise Authentik PostgreSQL scaling for production-scale deployments (12-core / 48 GB minimum hardware). Authentik upstream [#20714](https://github.com/goauthentik/authentik/issues/20714) is a confirmed bug — `django_channels_postgres` pool exhaustion at scale (each Authentik worker holds 1 LISTEN conn + 4 send-pool conns hardcoded at `layer.py:174`; ~100 Channels conns at baseline before any user activity). Tom Endress's `anctakserver2` forensic (2026-05-17) showed channels-conn baseline at 78/85 = 92% pool saturation under moderate load. Maintainer-endorsed mitigation (per the #20714 thread comments) until upstream fix lands in Authentik 2026.8.0 (~6 months out) is to raise capacity. v0.9.28 raises **`max_connections` 500 → 2000** + tunes PG memory for 48 GB hardware (`shared_buffers=12GB`, `effective_cache_size=36GB`, `work_mem=16MB`, `maintenance_work_mem=2GB`, `wal_buffers=64MB`, `max_wal_size=4GB`) + raises autotune ceiling (cap 60% → 75% = effective 1500-conn pool ceiling, **5× v0.9.27's 300-conn cap**) + raises fleet safe-constant 250/50 → **750/150** + raises pool floor 75/15 → **300/60** + bumps PgBouncer `MAX_CLIENT_CONN` 1000 → **5000**. New constant `_AUTHENTIK_PG_COMMAND_ENTERPRISE` is the single source of truth for the canonical PG command (referenced by install template + Update-Now migration paths — no more drift across 3 inline copies). Adds Channels-baseline telemetry (`peak_channels_idle`, `avg_channels_idle`, `samples_with_classes`) to `settings.json → pool_autotune.last_decision` for forensic drift tracking across releases. Removes the v0.9.27 operator-override carve-out in `_ensure_authentik_compose_patches` (`if max_connections > 500: preserve existing`) per `.cursor/rules/fleet-uniform-config.mdc` — every box converges to canonical enterprise command. Pool migration also reconciles `MAX_CLIENT_CONN` drift (a hotfix discovered during field validation: pool size at floor + stale `MAX_CLIENT_CONN=1000` was sticking after v0.9.27 → v0.9.28 update because pool-size-match short-circuited the pgbouncer recreate). **Field-validated across 3 dev boxes (test6, test8, test12): all at v0.9.28-alpha + enterprise PG tuning (max_connections=2000, shared_buffers=12GB, effective_cache_size=36GB, work_mem=16MB, maintenance_work_mem=2GB, wal_buffers=64MB, max_wal_size=4GB) + MAX_CLIENT_CONN=5000 + all 18 Authentik containers healthy + zero query_wait_timeout. test8 (Channels-heavy load, prior cl_waiting>0) at fleet-uniform cold-start 750/150 with peak_channels_idle=85 confirming Tom's observation. test6/test12 at autotune-floor 300/60 (correct for quiet load).** What does NOT ship (and why, briefly): patching the vendored `layer.py max_size=4` directly (fragile, ~40-conn savings, breaks at every Authentik upstream Update Now), switching CHANNEL_LAYERS to Redis (Authentik 2026.2.x hardcodes the PG backend in `/authentik/root/settings.py:328`; `channels_redis` not installed in the image; would require image fork), license-cache pre-warm (deferred to v0.9.29 — needs Authentik cache-key validation per release), worker-count reduction (wrong direction for enterprise throughput). See [release notes](docs/RELEASE-v0.9.28-alpha.md) for the full forensic + Authentik #20714 maintainer-thread cites.

Previous release: [v0.9.27-alpha](docs/RELEASE-v0.9.27-alpha.md) — Architectural fix for v0.9.26's silent fleet-fracture bug + four-layer self-healing pool management. v0.9.26's `_ensure_authentik_pgbouncer_pool_size` used `max(cur, target)` to "preserve operator overrides" — silently letting `tak-10` keep its operator-typed 250/50 while `test8` got the codified 75/15 → test8 hit 297 `query_wait_timeout` events in 5 min of pulling the release. **The 65-min validation done on tak-10 before shipping v0.9.26 was on a configuration the codebase never produced.** v0.9.27 ships in **four hotfix iterations**, each addressing a failure mode discovered live during dev validation across three test boxes (tak-10 + test8 + responder). (1) **Initial autotune + hotfix #1:** replace the v0.9.26 constant with a deterministic autotuner — pool size is a function of observed load on each box, computed identically everywhere (`target_ceiling = max(FLOOR=90, peak × 2.0)`, capped at PG_MAX_CONN × 0.6 = 300, split 5:1 default:reserve). Migration **always writes** the autotune target — no `max(cur, target)`. Cold-start (empty samples ring) uses a known-safe fleet constant 250/50 ceiling 300; replaces hotfix #0's unreliable single-shot live `pg_stat_activity` seed that under-sized tak-10 during a transient mid-restart traffic lull. Also fixes a stale `_reconcile_watchdog_threshold(new_default, new_reserve)` NameError. (2) **Hotfix #2 — cl_waiting demand telemetry + stuck-at-floor escape:** sampling idle-conn count alone measures pool *headroom* (it goes DOWN when pool is too small — clients can't check out → low idle → autotune confirms small pool is "fine," self-reinforcing under-sizing). New `_authentik_pgbouncer_cl_waiting()` queries `SHOW POOLS` inside the pgbouncer container for `cl_waiting + cl_waiting_cancel_req` — the direct demand signal that cannot lie under starvation. Autotune compute adds an escape: if ≥ 3 samples with cl_waiting telemetry have any > 0, force fleet safe-constant 250/50 regardless of idle peak. **Witnessed firing twice in production on test8** with `peak_cl_waiting=4 > 0 over 30 samples → forcing 250/50`. (3) **Hotfix #3 — continuous in-place remediation:** closes the architectural latency where hotfix #2's compute escape only ran at console restart (a quiet box getting a load spike would emit query_wait_timeouts until operator Update Now). Watchdog tick now triggers `_ensure_authentik_pgbouncer_pool_size()` directly when sustained starvation observed: 3 consecutive 2-min ticks with `cl_waiting > 0` AND `current_pool < 250` AND ≥ 15 min since last in-place resize → pgbouncer recreate in ~30s, no operator action required. **Detection-to-remediation: ~6.5 min worst case** (was unbounded). Triple-nested try/except so any failure here is swallowed and the original `idle > threshold → restart server-1` safety net always runs. (4) **Hotfix #4 — ghost-Channels-conn reaper:** test8 forensic 2026-05-17 22:14 UTC revealed 290 `SELECT DISTINCT django_channels_postgres_groupchannel` backends frozen in `state='idle'` for 2h 2min with consecutive PIDs — leaked by prior server-1 restarts (Authentik's Channels long-poll subscriptions don't unwind when the listener Python process dies, PgBouncer can't see the disconnect in transaction mode). Manually killed via `pg_terminate_backend` → idle 299→11, all containers healthy in <60s. Hotfix #4 ships this recovery: new `_authentik_reap_ghost_channels_conns()` runs the narrow termination SQL (`state='idle'` + groupchannel query + `state_change < NOW() - INTERVAL '5 min'`) from BOTH the startup migration (self-heals every v0.9.26 → v0.9.27 Update Now) AND the watchdog tick when `cl_waiting > 0` or `channels_idle > pool_ceiling/2` observed (40-min cooldown). **Field-validated: 20-min soak across three test boxes, all 18 Authentik containers (3 × 6) healthy with streak=0, zero tracebacks, zero query_wait_timeouts.** New `.cursor/rules/fleet-uniform-config.mdc` codifies the principle — no `max(cur, target)`-style override-preservation in any migration, ever; minimum 3-box dev-channel validation before main merge. See the [release notes](docs/RELEASE-v0.9.27-alpha.md) for the full four-incident forensic + the convergence behavior table.

Previous release: [v0.9.26-alpha](docs/RELEASE-v0.9.26-alpha.md) — Drop-in hotfix. Restores the v0.9.24-era "Authentik is perfectly quiet" baseline that v0.9.25's release-day restart-churn surfaced was no longer holding on `tak-10`. Shipped in **four hotfix waves + one amendment** the same day (2026-05-17), each driven by live forensic evidence from two field installs (tak-10 + Tom Endress's `anchortak`); v0.9.25's `cap_drop` heal is unchanged — purely additive. (1) **Initial:** new `_AUTHENTIK_TASKLOG_PURGE_SCRIPT` canonical constant + `_ensure_authentik_tasklog_purge_script` boot-time auto-heal + `_auto_authentik_tasklog_purge` inline Python cleanup wired into `_startup_migrations` — closes the v0.9.5 weekly `takauthentiktasklogpurge.service` timer that has been silently failing every Sunday for ~6 months with `ERROR: VACUUM cannot run inside a transaction block` and a hardcoded 30-day window that misses release-day churn (`authentik_tasks_*` reached ~960 MB / 2.66 M rows in <24 h on tak-10). (2) **Hotfix #2:** added `REINDEX TABLE CONCURRENTLY` + `VACUUM FULL` post-DELETE deep-compaction escalation + `_recreate_authentik_server_worker` to rescue installs where months of bloat left heap/indexes unable to shrink via `VACUUM ANALYZE` alone (tak-10: 960 MB → 1.3 MB, 740× reduction; LDAP bind 228 s → <1 ms). (3) **Hotfix #3:** bumped `DEFAULT_POOL_SIZE` 35 → **75** and `RESERVE_POOL_SIZE` 5 → **15** (90-conn ceiling) after diagnosing that **Django Channels' PG-backed `groupchannel` long-polling SELECTs** were eating ~17 of 35 pool slots → `query_wait_timeout` storms → 8446 routing `webadmin` to WebTAK landing. (4) **Hotfix #4** (incorporates Tom Endress's `anchortak` 2026-05-17 05:30 UTC incident report): new `_ensure_vm_overcommit_memory()` startup migration sets `vm.overcommit_memory = 1` runtime + `/etc/sysctl.conf` so Redis BGSAVE forks don't fail with `ENOMEM` and trigger `stop-writes-on-bgsave-error`; `ak-pg-watchdog` extended with `_classify_idle_load()` so every fire shows the breakdown (`Channels=89 dramatiq=14 cache=22 advisory_lock=3 other=34 (dominant=Channels)`) — gives operators class-level visibility without ad-hoc `pg_stat_activity`. (5) **Hotfix #4 amendment:** new `_reconcile_watchdog_threshold()` inside `_ensure_authentik_pgbouncer_pool_size` makes the watchdog's idle-connection threshold scale with the PgBouncer pool ceiling (`ceiling + 50`) — closes the false-restart-loop discovered on tak-10 where the hardcoded 150 threshold was firing on a healthy 180-conn pre-warmed pool. **Field-validated on tak-10: 1 h 5 min of continuous stability post-amendment** (zero `query_wait_timeout`, zero `context canceled`, all containers healthy). See the [release notes](docs/RELEASE-v0.9.26-alpha.md) for the full forensic trace + tak-10 evidence. Older releases on the [GitHub Releases tab](https://github.com/takwerx/infra-TAK/releases).
