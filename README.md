...
### v0.9.35-alpha — 2026-05-20 — Node-RED Emergency Config Restore "Failed to fetch" fix + CloudTAK update checkout failure fix

**Headline: two independent bugfixes.** (1) Clicking Refresh in the Emergency Config Restore panel always failed with "Error loading backups: Failed to fetch" on any Authentik-protected deployment. Root cause confirmed on tak-10: Authentik's 1-hour proxy session token expires, Caddy's `forward_auth` returns HTTP 302 to `tak.<domain>` (cross-origin), the browser's `fetch()` follows the redirect, hits a CORS block at the Authentik OAuth page, and throws `TypeError: Failed to fetch`. The `GET /config/backups` Node-RED endpoint was always healthy — the failure was purely in how the Configurator handled the auth-redirect case. Fix: `loadBackupList()` and `restoreBackup()` now use `_fetchT()` with `redirect: 'manual'` (consistent with all other Configurator API calls). With `redirect: 'manual'`, Caddy's 302 produces an `opaqueredirect` response (`status === 0`) instead of a CORS-blocked network error. The code detects this and shows a clear amber message: **"Session expired — reload the page to re-authenticate, then click Refresh again."** When the session is live, the backup list loads exactly as before.

(2) CloudTAK Update button failed with "Your local changes to the following files would be overwritten by checkout" when the CloudTAK working tree had any locally modified tracked files. Root cause: `run_cloudtak_update()` ran `git checkout {tag}` without first resetting local modifications — the TAK Portal update has always done `git checkout -- .` before its pull; CloudTAK's update path was missing the equivalent step. Fix: `git checkout -- .` added before the fetch+checkout in both the local and remote update paths. CloudTAK's persistent data (DB volume, `.env`, override compose) is untracked and is not affected.
